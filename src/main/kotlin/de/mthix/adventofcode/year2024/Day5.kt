...
fun main() {
    ...
    val pages = linesOfDay(2024,5, example)
        .map { it.split(",").map { it.toInt() } }

    val orderedPages = pages.filter { order.isOrdered(it) }
    println(orderedPages.map { it[it.size/2] }.sum())
        ...
        .onEach { pageSet ->
            while(!order.isOrdered(pageSet)) {
                ...
                while(i < pageSet.size) {
                    //println("i:$i current:${pageSet[i]} pages:$pageSet sublist${pageSet.subList(i, pageSet.size)}")
    ...
    println(unorderedPages.map { it[it.size/2] }.sum())
}
